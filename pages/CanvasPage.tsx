            ...
            summaryColumns.forEach(col => {
                ...
                try {
                    if (col === 'suggestedCapability') {
                        ...
                        if (col === 'yearlyScore_5') {
                            // For 2029, use the same logic as agreedFor5Years
                            let refinedScore = 0;
                            if (hasAllInputs) {
                                const df4Result = calculateScoresForSingleFactor(allInputs, 'df4').find(r => r.objectiveId === obj.id);
                                refinedScore = df4Result?.finalScore || 0;
                            ...
                            // Only calculate if user has input a value for other yearly scores
                ...
                if (typeof value === 'number' && !isNaN(value)) {
                    if (col === 'initialQuickScoring' || (col.startsWith('yearlyScore_') && col !== 'yearlyScore_5')) {
                        ...
                        <table className="border-collapse text-xs">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                ...
                                {canvasData.map((row) => {
                                    ...
                                    // Ensure concludedScope is a valid number
                                    const finalConcludedScope = typeof concludedScope === 'number' && !isNaN(concludedScope) ? concludedScope : 0;
                                    const suggestedCapability = calculateSuggestedCapabilityLevel(finalConcludedScope);
                                    ...
                                    // Ensure agreedCapabilityValue is always a valid number
                                    const finalAgreedCapabilityValue = typeof agreedCapabilityValue === 'number' && !isNaN(agreedCapabilityValue) ? agreedCapabilityValue : suggestedCapability;
                                    ...
                                    return (
                                        <tr key={row.id}>
                                            ...
                                            <td className="p-1 whitespace-nowrap text-center font-mono font-bold bg-yellow-50"><DivergingBarCell value={finalConcludedScope} maxAbsValue={maxConcludedScope} /></td>
                                            ...
                                            <td className="p-1 whitespace-nowrap bg-yellow-50" onClick={() => !isEditingAgreedCapability && setEditingCell(agreedCapabilityCellId)}>
                                                {isEditingAgreedCapability ? (
                                                    ...
                                                    <CapabilityBarCell value={finalAgreedCapabilityValue} max={4} />
                                                )}
                                            </td>
                                            ...
                                            {step5Cols.map(col => {
                                                ...
                                                if (col.id === 'agreedFor5Years' || col.id === 'yearlyScore_5') {
                                                    // For 2029, always use the agreed capability value
                                                    displayValue = finalAgreedCapabilityValue;
                                                    isReadOnly = true;
